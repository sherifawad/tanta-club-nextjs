...
export const playersWithMaxDiscountSorting = (
    players: Player[],
    step: number = 0
): Player[] => {
...
export const getPlayersWithPrivateSwimmingTimeDiscount = (
    players: Player[]
): Player[] => {
    return players.filter((player) =>
        player.sports.filter((sport) =>
            sport.discounts?.find((discount) => discount.id === 5)
        )
    );
};
...
export const mergePlayers = (
    ...
    return players1
        ...
        .concat(
            players2.filter((item) =>
                players1.every((x) => x.name !== item.name)
            )
        );
};
