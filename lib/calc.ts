...
export const onePlayer = (player: Player) => {
    ...
    if (sportsWithDiscount.length === 1) {
        ...
        const discount = sportsWithDiscount[0].discounts![0];
        ...
        if (sportsWithDiscount) {
            ...
            // 	sport.discounts?.some((discount) => discount.id === 1 || discount.id === 2)
            // );
...
export const twoPlayers = (players: Player[]): Player[] => {
    ...
    const haveManySports = players.every(
        (p) => numberOfSportsWithDiscount(p).length > 1
    );
    ...
    if (haveManySports) {
        ...
        const isSecondSportBigger = firstSportsWithDiscountBigger(
            players[1].sports[0],
            players[0].sports[0]
        );
        ...
        const playerIndexWithMultiple = players.findIndex(
            (x) => numberOfSportsWithDiscount(x).length > 1
        );
        ...
        if (playerIndexWithMultiple === -1) {
            ...
            const lowerPlayer = players.findIndex(
                (x) => numberOfSportsWithDiscount(x).length === 1
            );
...
export const moreThanTwoPlayers = (players: Player[]): Player[] => {
    const [playersWithDiscountSports, otherPlayers] = divvyUp(
        players,
        (player) =>
            player.sports.find((sport) =>
...
export const swimmingDiscount = (players: Player[]): Player[] => {
    ...
    let [otherSwimming, sportsWithBrothersDiscount] = divvyUp(
        players,
        (player) =>
            player.sports.find((sport) =>
                sport.discounts?.every((discount) => discount.id === 5)
            )
    );
    let otherSwimmingWithoutBrotherDiscount: Player[] = [];
    let otherSwimmingWithBrothersDiscount: Player[] = [];
    otherSwimming.forEach((player) => {
        let [hasNoBrotherDiscount, hasBrotherDiscount] = divvyUp(
            player.sports,
            (sport) => sport.discounts?.every((discount) => discount.id !== 6)
        );
    ...
    sportsWithBrothersDiscount = [
        ...sportsWithBrothersDiscount,
        ...otherSwimmingWithBrothersDiscount,
    ].filter((player) => player.sports.length !== 0);

    if (
        otherSwimmingWithoutBrotherDiscount &&
        otherSwimmingWithoutBrotherDiscount.length > 0
    ) {
        otherSwimmingWithoutBrotherDiscount =
            otherSwimmingWithoutBrotherDiscount.map((player) => {
